...
pub trait JarExt {
    ...
    fn load_netscape(path: &impl AsRef<Path>) -> crate::Result<Jar>;
}

impl JarExt for Jar {
    fn load_netscape(path: &impl AsRef<Path>) -> crate::Result<Self> {
        ...
        while reader.read_line(&mut line)? > 0 {
            ...
            let domain = parts
                .next()
                .ok_or_else(|| crate::Error::NotFound("malformed cookie: missing domain".into()))?;
            let key = parts
                .nth(4)
                .ok_or_else(|| crate::Error::NotFound("malformed cookie: missing name".into()))?;
            let value = parts
                .next()
                .ok_or_else(|| crate::Error::NotFound("malformed cookie: missing value".into()))?;
