...
impl UserRates {
    ...
    pub const fn new(
        ...
        Self {
            user_rate: UserRatesUserRate::new(episodes, rewatches, status, target_id, target_type, user_id),
        }
    }
    ...
    pub async fn patch(&self, client: &Client, host: &str, user_rate_id: usize) -> crate::Result<UserRate> {
        let user_rate = client
            .patch_json_as_json(
                &kodik_utils::api_url(host, &format!("/api/v2/user_rates/{user_rate_id}")),
                self,
            )
            .await?;
    ...
    pub async fn post(&self, client: &Client, host: &str) -> crate::Result<UserRate> {
        let user_rate = client
            .post_json_as_json(&kodik_utils::api_url(host, "/api/v2/user_rates"), self)
            .await?;
